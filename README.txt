...
- Expose an API endpoint where I can GET stock market information, as a security mechanism use the API key obtained previously in order to 
  validate user and make sure that no authorized user will consume the service (use request header for that purpose).

Here are some examples of stock symbols: Facebook (FB), Apple (AAPL), Microsoft (MSFT), Google (GOOGL), Amazon (AMZN)
...
--------------------------------------------------------
Alpha Vantage API
https://www.alphavantage.co/documentation/      API Key: X86NOH6II01P7R24
...
1 - Create POST and get key     --> Done
2 - Create GET                  --> Done
3 - Check if user exist         --> Done        
4 - Add number of rows in GET   --> Done
5 - Add handler for errors      --> Done
6 - Save parameters in DB       --> Done
7 - Get Key from DB             --> Done
8 - Logging                     --> Done
